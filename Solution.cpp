...
class Solution {
    ...
    static const int DIGIT_TO_INSERT_FOR_MIN_VALUE_WHEN_LEADING_DIGIT_IS_EQUAL_TO_ONE = 0;
    static const int DIGIT_TO_INSERT_FOR_MIN_VALUE_WHEN_LEADING_DIGIT_IS_GREATER_THAN_ONE = 1;
    static const int DIGIT_TO_INSERT_FOR_MAX_VALUE = 9;

public:
    int maxDiff(int input) const {
        int numberOfDigits = 1 + log10(input);
...
private:
    int createMinValueWithoutLeadingZerosAndGreaterThanZero(int input, int numberOfDigits) const {
    ...
    int createMaxValue(int input, int numberOfDigits) const {
        int maxValue = 0;
        int digitToReplace = NOT_FOUND;
        int extractorForLeftmostValue = pow(10, numberOfDigits - 1);
